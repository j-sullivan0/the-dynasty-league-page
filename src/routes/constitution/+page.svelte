...
<div class="constitution">
    <h1 class="noUnderscore">THE DYNASTY LEAGUE CONSTITUTION</h1>
    ...
    <h3 class="noUnderscore clickable" on:click={() => goToSection(one)}>Section 1: Roster</h3>
        ...
        <h4 class="noUnderscore clickable" on:click={() => goToSection(oneTwo)}>1.2 Trading</h4>
            <h5 class="noUnderscore clickable" on:click={() => goToSection(oneTwoOne)}>1.2.1 Trade Deadline</h5>
            <h5 class="noUnderscore clickable" on:click={() => goToSection(oneTwoTwo)}>1.2.2 Trade Restrictions</h5>
            <h5 class="noUnderscore clickable" on:click={() => goToSection(oneTwoThree)}>1.2.3 Veto/Trade Process</h5>
    ...
    <div class="subBlock">
        <p>The trade deadline will be set for Week 11 - No trades after Week 11.</p>
    </div>
    ...
    <div class="subBlock">
        <p>A total of 5 veto votes are required to veto a trade. Vetoes are only appropriate in instances of collusion or an egregious mismatch of value (i.e. CMC for a 3rd string player). Not liking a trade does not warrant a veto.</p>
    </div>
    ...
    <p>At the end of the season 6 teams will make the playoffs. The first round of playoffs will commence in week 15. The top 6 overall records will receive a playoff berth. The top 2 teams will receive a bye week in the first round of the playoffs. Each playoff matchup will only last one week and the winner will advance. Week 18 will not be used.</p>
    ...
    <p>League dues are set at {dues}$. Dues are collected via multiple payment methods (mainly Venmo or equivalent) and collection will be sent out no later than the first of September. All managers must be fully paid up before the start of regular season. Failure to make payment is grounds for removal, at the League's discretion</p>
    
    <h3 bind:this={sevenTwo}>7.2 Payout</h3>
    <p>League payout is winner-takes-all. The Champion will receive 100% of the payout.</p>
    
    <p>If new managers come into the league and are offered a 50% discount, the payout will be reduced by this difference.</p>
